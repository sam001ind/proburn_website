...
export const getSectionStyle = (data, defaultBg = 'var(--bg)') => {
...
export const HeroBlock = ({ data }) => (
  <section style={{ 
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
      <h1 style={{ fontSize: '4rem', fontWeight: 800, marginBottom: '1rem', color: 'var(--accent)' }}>{data.title || 'Hero Title'}</h1>
      <p style={{ fontSize: '1.5rem', marginBottom: '2rem', opacity: 0.9, maxWidth: '600px' }}>{data.subtitle || 'Hero Subtitle'}</p>
      {data.buttonText && (
  ...
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--surface)') }}>
    <div style={{ 
      ...
      <div style={{ order: data.imagePosition === 'left' ? 2 : 1 }}>
        <h2 style={{ fontSize: '2.5rem', marginBottom: '1.5rem', color: 'var(--accent)' }}>{data.title || 'Title'}</h2>
        <p style={{ fontSize: '1.1rem', lineHeight: 1.6, opacity: 0.8 }}>{data.text || 'Text content'}</p>
      </div>
      {data.image && (
...
export const FeaturesGridBlock = ({ data }) => (
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--bg)') }}>
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <h2 style={{ textAlign: 'center', fontSize: '2.5rem', marginBottom: '3rem', color: 'var(--accent)' }}>{data.title || 'Features'}</h2>
      <div style={{ 
...
export const RichTextBlock = ({ data }) => (
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--bg)') }}>
    <div 
      ...
      dangerouslySetInnerHTML={{ __html: data.content || '<p>Rich Text content</p>' }}
    />
  </section>
);

export const PricingBlock = ({ data }) => (
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--surface)') }}>
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <h2 style={{ fontSize: '2.5rem', color: 'var(--accent)', marginBottom: '0.5rem' }}>{data.title || 'Pricing'}</h2>
...
export const ScheduleBlock = ({ data }) => (
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--bg)') }}>
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <h2 style={{ fontSize: '2.5rem', color: 'var(--accent)', marginBottom: '0.5rem' }}>{data.title || 'Schedule'}</h2>
        {data.subtitle && <p style={{ fontSize: '1.1rem', opacity: 0.8 }}>{data.subtitle}</p>}
      </div>
...
export const ContactBlock = ({ data }) => (
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--surface)') }}>
    <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gap: '3rem', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
      <div>
        <h2 style={{ fontSize: '2.5rem', color: 'var(--accent)', marginBottom: '0.5rem' }}>{data.title || 'Contact'}</h2>
...
export const TestimonialsBlock = ({ data }) => (
  <section style={{ padding: '4rem 2rem', ...getSectionStyle(data, 'var(--bg)') }}>
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <h2 style={{ textAlign: 'center', fontSize: '2.5rem', marginBottom: '3rem', color: 'var(--accent)' }}>{data.title || 'Testimonials'}</h2>
...
export const CarouselBlock = ({ data }) => {
  ...
  if (images.length === 0) return <section style={{ padding: '2rem', textAlign: 'center', ...getSectionStyle(data, 'var(--bg)') }}>No images in carousel</section>;
  return (
...
export const YoutubeBlock = ({ data }) => {
...
export const DividerBlock = ({ data }) => {
  const styleMap = {
...
export const MultiColumnBlock = ({ data }) => {
